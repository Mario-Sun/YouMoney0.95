...
from ui.loader import load_bitmap
import datetime
import shutil
...
import version
from ui.storage import catetypes, payways, cycles
import gettext
...
class MainFrame (wx.Frame):
    def __init__(self, parent, id, title, cf):
        ...
        icon.CopyFromBitmap(wx.Bitmap(wx.Image(iconpath, wx.BITMAP_TYPE_PNG)))
        self.SetIcon(icon)
        self.CenterOnScreen()
...
#       print 'count:', count, 'iscreate:', config.cf.iscreate, 'lang:', config.cf['lang']
    ...
    def notify(self):
        ...
        if sys.platform.startswith('win32') and lastdb.startswith(os.environ['SystemDrive']) \
                and self.conf.lastdb_is_default():
            wx.MessageBox(_('You db file is in default path, strongly advise save it to other path. '
                            'Choose menu File->Change Account Path to change path.'), _('Note:'),
...
#       check record cycle
    ...
    def make_menu(self):
        ...
        self.filemenu.AppendSeparator()
        self.filemenu.Append(self.ID_FILE_IMPORT, _('Import'), self.importmenu)
        self.filemenu.Append(self.ID_FILE_EXPORT, _('Export'), self.exportmenu)
        self.filemenu.AppendSeparator()
        ...
        self.viewmenu.Append(self.ID_VIEW_LANG, _('Language'), self.langmenu)
    ...
    def make_toolbar(self):
        ...
        # shortHelp=_('Add Category'), longHelp=_('Add Category'))
        self.toolbar.AddTool(self.ID_TB_CATEEDIT, _('Add Category')+'(&T)', load_bitmap(os.path.join(self.bmpdir, 'categories.png')), wx.NullBitmap, shortHelp=_('Add Category'), longHelp=_('Add Category'))
        self.toolbar.AddTool(self.ID_TB_INCOME, _('Add Income')+'(&I)', load_bitmap(os.path.join(self.bmpdir, 'cashin.png')),
                             wx.NullBitmap, shortHelp=_('Add Income'), longHelp=_('Add Income'))
        self.toolbar.AddTool(self.ID_TB_PAYOUT, _("Add Payout")+'(&P)', load_bitmap(os.path.join(self.bmpdir, 'cashout.png')), wx.NullBitmap, shortHelp=_("Add Payout"), longHelp=_("Add Payout"))
        self.toolbar.AddTool(self.ID_TB_CYCLE, _("Add Record Cycle")+'(&C)', load_bitmap(os.path.join(self.bmpdir, 'cycle.png')), wx.NullBitmap, shortHelp=_("Record Cycle"), longHelp=_("Record Cycle"))
