			...
			while self._peek() != '"':
				...
				if ch == "\\":
					...
					if ch in 'brnft':
						...
						raise ReadException(
							"Not a valid escaped JSON character: '%s' in %s" % (ch, self._generator.all()))
				result = result + ch
		...
		try:
			...
			while not done:
				...
				if not done and ch == "/" and self._peek() == "*":
					raise ReadException(
						"Not a valid JSON comment: %s, '/*' cannot be embedded in the comment." % self._generator.all())
			self._next()
		except StopIteration:
...
class JsonWriter(object):
	...
	def _write(self, obj):
		...
		if self._escaped_forward_slash:
			...
			self._append(obj)
			self._append('"')
		elif ty is int or ty is int: # elif ty is int or ty is int
