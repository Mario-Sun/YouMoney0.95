...
class CategoryPanel(wx.Panel):
        ...
        if data['id'] > 0:
            ...
            if ret:
                ...
                if row['type'] == 0:
                    ...
                    ct = _('Income')
                ready = {'cates': [], 'cate': row['name'], 'upcate': upcate, 'catetype': ct, 'mode': 'update',
                         'id': row['id']}
    ...
    def OnCategoryDel(self, event):
        ...
        if name == _('No Category'):
            dlg = wx.MessageDialog(self, _('Can not delete this category!'), _('Notice:'), wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
            return
        else:
            dlg = wx.MessageDialog(self, _('To delete category') + '"' + name + '"' + _(
                ', all child categories will move to "No Category"'), _('Notice:'), wx.OK | wx.ICON_INFORMATION | wx.CANCEL)
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                frame.db.rollback()
                wx.MessageBox(_('Delete category failure!') + str(e), _('Delete category information'),
        ...
        if rets:
            ...
            for row in rets:
                ...
                if self.type == 'payout':
                    ...
                    self.itemDataMap[row['id']] = [mytime, cate, str(row['num']), payway, cyclestr.encode('utf-8'),
                                                   row['explain']]
                else:
