...
def _visit_pyfiles(list, dirname, names):
    ...
    if '_py_ext' not in globals():
        global _py_ext
        _py_ext = [triple[0] for triple in importlib.get_suffixes()     # TODO: deprecated
        ...
        try:
            ...
            try:
                ...
                print('%s: %s, line %d, column %d' % (
                    e[0], filename, e[1][0], e[1][1]), file=sys.stderr) #FIXME: Class 'TokenError' does not define '__getitem__', so the '[]' operator cannot be used on its instances.
        finally:
            if closep:
                fp.close()
